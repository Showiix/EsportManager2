...
### 2.8 数据中心系统 (MatchDetailSystem)

数据中心系统负责持久化存储比赛详情数据，包括每局比分、选手表现、MVP信息、激活的特性等。
...
CREATE TABLE IF NOT EXISTS game_player_performances (
    ...
    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (game_id) REFERENCES match_games(id)
);

CREATE INDEX idx_match_games_save_match ON match_games(save_id, match_id);
CREATE INDEX idx_performances_game ON game_player_performances(game_id);
```
...
pub struct GamePlayerPerformance {
    ...
    pub team_id: i64,
    pub position: String,
    pub impact_score: f64,      // 影响力分数 (0-100)
    pub actual_ability: f64,    // 实际发挥能力值
    pub is_mvp: bool,           // 是否为本局MVP
    pub is_key_player: bool,    // 是否为关键选手
}

/// 激活的特性信息 (用于前端展示)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ActivatedTraitInfo {
    pub trait_type: String,     // 特性类型 (如 "ClutchPlayer")
    pub name: String,           // 显示名称 (如 "关键先生")
    pub effect: String,         // 效果描述 (如 "状态 +5")
    pub value: f64,             // 效果数值
    pub is_positive: bool,      // 是否为正面效果
}
...
impl MatchGameDetailRepository {
    ...
    pub async fn save_match_details(
        ...
        // 1. 删除已有数据 (支持重新模拟)
        Self::delete_match_details_tx(&mut tx, save_id, input.match_id).await?;

        // 2. 插入每局数据
        for game in &input.games {
            let game_id = Self::insert_game(&mut tx, save_id, input.match_id, game).await?;

            // 3. 插入选手表现数据
            for perf in &game.performances {
                Self::insert_performance(&mut tx, save_id, &game_id, perf).await?;
            }
        }
    ...
    pub async fn get_match_full_details(
        ...
        save_id: &str,
        match_id: i64,
    ) -> Result<Option<MatchFullDetails>, Error>;
...
export const matchDetailsApi = {
  // 保存比赛详情到数据库
  saveMatchDetails: async (saveId: string, input: SaveMatchDetailsInput): Promise<void> => {
    const result = await invoke<CommandResult<void>>('save_match_details', {
      saveId,
      input
    })
  ...
  // 从数据库获取比赛详情
  getMatchDetails: async (saveId: string, matchId: number): Promise<MatchFullDetails | null> => {
    const result = await invoke<CommandResult<MatchFullDetails | null>>('get_match_details', {
      saveId,
      matchId
    })
    if (!result.success) throw new Error(result.error)
  ...
  // 删除比赛详情
  deleteMatchDetails: async (saveId: string, matchId: number): Promise<void> => {
    const result = await invoke<CommandResult<void>>('delete_match_details', {
      saveId,
      matchId
    })
    if (!result.success) throw new Error(result.error)
...
比赛模拟完成
    ↓
生成 DetailedMatchResult (包含 PlayerGameStats)
    ↓
前端 Store 调用 saveMatchDetail()
    ↓
转换为 SaveMatchDetailsInput 格式
    ↓
调用 matchDetailsApi.saveMatchDetails()
    ↓
Tauri IPC → Rust 后端
    ↓
MatchGameDetailRepository.save_match_details()
    ↓
事务写入 match_games + game_player_performances 表
    ↓
返回成功

查看历史比赛详情
    ↓
前端调用 matchDetailsApi.getMatchDetails()
    ↓
Tauri IPC → Rust 后端
    ↓
MatchGameDetailRepository.get_match_full_details()
    ↓
JOIN 查询组装 MatchFullDetails
    ↓
返回完整比赛数据 (包含每局详情和选手表现)
```
...
| 字段 | 说明 | 计算方式 |
|------|------|----------|
| impact_score | 影响力分数 | 基于实际发挥值的标准化评分 (0-100) |
| actual_ability | 实际发挥 | 基础能力 + 状态加成 + 高斯噪声 + 特性修正 |
| is_mvp | 本局MVP | 影响力分数最高的选手 |
| is_key_player | 关键选手 | 由比赛逻辑指定的关键表现选手 |

#### 2.8.8 特性系统集成

比赛模拟时会计算每位选手激活的特性，并通过 `PlayerGameStats.activated_traits` 字段返回前端：

```rust
// 特性激活条件示例
match trait_type {
    TraitType::ClutchPlayer if is_playoff => ("状态 +5", 5.0, true),
    TraitType::BigGamePlayer if is_international => ("状态 +4", 4.0, true),
    TraitType::ComebackKing if score_diff < 0 => ("状态 +3", 3.0, true),
    TraitType::Tilter if score_diff > 0 => ("状态 -2", -2.0, false),
    TraitType::RisingStar if is_first_season => ("能力 +3", 3.0, true),
    TraitType::Veteran if age >= 30 => ("稳定性 +15", 15.0, true),
    // ... 其他特性
}
```
