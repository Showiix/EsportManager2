use super::traits::{TraitEngine, TraitType};
...
pub struct RookieGenerationConfig {
    pub tag_override: Option<String>,
    pub ability_range: Option<(u8, u8)>,
    pub potential_range: Option<(u8, u8)>,
    pub position_override: Option<String>,
    pub traits_override: Option<Vec<String>>,
}
...
impl RookieGenerator {
    ...
    pub fn generate_rookies(
        ...
        self.generate_rookies_with_config(
            region_id,
            count,
            existing_ids,
            &RookieGenerationConfig::default(),
        )
    }

    /// 带配置的新秀生成
    pub fn generate_rookies_with_config(
        &mut self,
        region_id: u64,
        count: usize,
        existing_ids: &HashSet<String>,
        config: &RookieGenerationConfig,
    ) -> Vec<GeneratedRookie> {
        ...
        for i in 0..count {
            ...
            let tag = match &config.tag_override {
                Some(t) => t.clone(),
                None => self.roll_tag(),
            };
            let age = self.roll_age();

            let ability = match config.ability_range {
                Some((min, max)) => self.rng.gen_range(min..=max),
                None => self.roll_ability(&tag),
            };

            let potential = match config.potential_range {
                Some((min, max)) => self.rng.gen_range(min..=max).max(ability),
                None => self.roll_potential(ability, &tag),
            };

            let stability = self.roll_stability(age);

            let position = match &config.position_override {
                Some(p) => p.clone(),
                None => positions[i % positions.len()].to_string(),
            };

            let nationality = get_nationality(region_id).to_string();

            let traits = match &config.traits_override {
                Some(trait_names) => trait_names
                    .iter()
                    .filter_map(|name| TraitType::from_str(name))
                    .collect(),
                None => TraitEngine::generate_random_traits(ability, age, &mut self.rng),
            };
...
fn get_lck_game_ids() -> Vec<&'static str> {
    vec![
        "Kaze", "Onyx", "Dusk", "Void", "Apex", "Nox", "Zen", "Ark", "Ryx", "Sol", "Hex", "Ion",
        "Aero", "Crux", "Zeta", "Cyan", "Aeon", "Oryx", "Flux", "Byte", "Nero", "Jax", "Vyn",
        "Kayn", "Yoru", "Dex", "Elm", "Grim", "Koda", "Mars", "Navi", "Odin", "Prax", "Riku",
        "Shin", "Tobi", "Wren", "Xero", "Yuki", "Zael", "Aven", "Bane", "Ciel", "Dion", "Ezra",
        "Fael", "Gale", "Hawk", "Izon", "Jinx", "Kael", "Lynx", "Miru", "Nova", "Omen", "Pike",
        "Quil", "Rift", "Sage", "Tyr", "Ulti", "Vex", "Wynn", "Xion", "Yato", "Zion", "Arik",
...
fn get_lec_game_ids() -> Vec<&'static str> {
    vec![
        ...
        "Nolan", "Olaf", "Pelle", "Rafael", "Stefan", "Tomas", "Adrian", "Boris", "Cyril",
        "Dorian", "Edgar", "Fabian", "Gustav", "Henrik", "Johan", "Klaus", "Lorenz", "Milan",
        "Niklas", "Oskar", "Pascal", "Romain", "Soren", "Trond", "Urban", "Viktor", "Walden",
        "Xander", "Yngvar", "Zoran", "Albin", "Bjorn", "Casper", "Dieter", "Edvin", "Frode",
        "Gunnar", "Halvar", "Ivar", "Joris", "Konrad", "Ludwig", "Magnus", "Norbert", "Orsino",
        "Pierre", "Rainer", "Sander", "Theron", "Ulric", "Varek", "Werner", "Xenos", "Yannick",
        "Zorion", "Aldric", "Baldur", "Cedric", "Darius", "Emeric", "Fenris", "Geralt", "Hadric",
...
fn get_lcs_game_ids() -> Vec<&'static str> {
    vec![
        "Brix", "Colt", "Duke", "Edge", "Fury", "Grif", "Holt", "Indy", "Jace", "Kade", "Lark",
        "Mace", "Nile", "Oaks", "Pace", "Raid", "Slade", "Tank", "Vick", "Wynn", "York", "Zeke",
        "Asher", "Blake", "Cliff", "Drew", "Ford", "Grant", "Hayes", "Jude", "Knox", "Lane",
        "Miles", "Nash", "Orion", "Pike", "Quinn", "Reed", "Stone", "Trace", "Wade", "Axel",
        "Brock", "Chase", "Dane", "Flint", "Grit", "Hawk", "Ivan", "Jet", "Kane", "Lobo", "Mavic",
        "Noel", "Owen", "Puma", "Rock", "Scout", "Thor", "Uno", "Vance", "Wolf", "Xray", "Yogi",
...
fn get_lpl_game_ids() -> Vec<&'static str> {
    vec![
        "Arch", "Davi", "Eddy", "Fang", "Glen", "Ives", "Jago", "Kino", "Loki", "Mako", "Noel",
        "Puma", "Qing", "Rex", "Sven", "Tao", "Vega", "Xing", "Yang", "Alto", "Cozy", "Dawn",
        "Grit", "Halo", "Iris", "Jade", "Keen", "Muse", "Nord", "Onyx", "Peak", "Rein", "Sage",
        "Tide", "Umber", "Vale", "Wing", "Yato", "Zinc", "Bliss", "Crow", "Daze", "Echo", "Flux",
        "Glow", "Hive", "Ivy", "Jazz", "Kite", "Loom", "Mesa", "Nite", "Opal", "Plum", "Rune",
        "Silk", "Tusk", "Ursa", "Vine", "Welt", "Xeno", "Yew", "Zeal", "Aqua", "Brim", "Clay",
...
fn get_lck_name_pool() -> (&'static [&'static str], &'static [&'static str]) {
    static SURNAMES: &[&str] = &[
        "金", "李", "朴", "崔", "郑", "安", "姜", "申", "林", "徐", "黄", "闵", "赵", "权", "宋",
    ];
    static GIVEN_NAMES: &[&str] = &[
        "宇烈", "承焕", "敏锡", "志勋", "建佑", "宰贤", "道贤", "仁浩", "珉宇", "贤宇", "成浩",
        "成民", "晙赫", "泰勋", "晙奭", "正宇", "俊熙", "在宇", "承民", "尚赫", "民奎", "泰贤",
        "秀焕", "东辉", "炫宇", "圣勋", "瑞民", "柱焕", "相赫", "星旿",
    ];
