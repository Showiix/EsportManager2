...
import { DataLine, Refresh, Plus } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { usePlayerStore } from '@/stores/usePlayerStore'
import { useMatchDetailStore } from '@/stores/useMatchDetailStore'
import type { PlayerPosition, Player } from '@/types/player'
import { POSITION_NAMES } from '@/types/player'
import { PowerEngine } from '@/engines'
import { statsApi, type PlayerRankingItem } from '@/api/tauri'
...
const loadRankingsFromDB = async () => {
  ...
  try {
    // 将赛季字符串转换为数字
    const seasonNum = parseInt(selectedSeason.value) || 1
...
const generateMockData = async () => {
  ...
  try {
    ...
    // 为每个队伍生成5名选手
    const generateTeamPlayers = (teamId: string, _teamName: string): Player[] => {
